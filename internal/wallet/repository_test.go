...
func TestGetOrCreateWallet_WhenNotExists(t *testing.T) {
    ...
    w, err := repo.GetOrCreateWallet(context.Background(), 10)
    require.NoError(t, err)
...
func TestAddTransaction_Success_UpdateAndInsert(t *testing.T) {
    ...
    // UPDATE wallets
    mock.ExpectExec(regexp.QuoteMeta("UPDATE wallets\n\t\t SET balance_cents = $1, updated_at = NOW()\n\t\t WHERE id = $2")).
        WithArgs(1500, 7).
        WillReturnResult(sqlmock.NewResult(0,1))

    // INSERT wallet_transactions
    mock.ExpectExec(regexp.QuoteMeta("INSERT INTO wallet_transactions (wallet_id, amount_cents, type, balance_after)\n\t\t VALUES ($1, $2, $3, $4)")).
